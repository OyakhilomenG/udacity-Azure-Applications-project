...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            app.logger.warning('Invalid login attempt by user: %s', form.username.data)
            flash('Invalid username or password')
            return redirect(url_for('login'))
        login_user(user, remember=form.remember_me.data)
        app.logger.info('User successfully logged in: %s', form.username.data)
...
def authorized():
    ...
    if request.args.get('code'):
        ...
        # Done
        result = _build_msal_app(cache=cache).acquire_token_by_authorization_code_flow(session.get("flow", {}), request.args)
        if "error" in result:
